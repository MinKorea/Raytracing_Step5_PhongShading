...
namespace hlab
{
	...
	public:
			...
			: width(width), height(height)
		{
			...
			light = Light{{0.0f, 0.0f, -1.0f}}; // Behind of the screen
		}
		...
		vec3 traceRay(Ray &ray)
		{
			...
			else
			{
				...
				const float diff = glm::max(glm::dot(dirToLight, hit.normal), 0.0f);

				// Specular
				const vec3 reflectDir = 2.0f * glm::dot(hit.normal, dirToLight) * hit.normal - dirToLight;
				// r = 2 (n dot l) n - l
